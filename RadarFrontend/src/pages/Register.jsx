...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setStatus('success');
      setMessage(res.data.message || 'Verification email has been sent. Please check your inbox.');
    } catch (error) {
      setStatus('idle');
    ...
    <AuthLayout>
      {status === 'success' ? (
        ...
        >
          ...
          <a
            href="/login"
            className="inline-block bg-[#10706B] text-white font-bold py-4 px-8 rounded-xl hover:bg-[#0D5C58] transition-all shadow-lg text-sm tracking-[0.1em] uppercase"
          >
            Go to Login
          </a>
        </motion.div>
          ...
          <div className="space-y-4">
            <form onSubmit={handleSubmit} className="space-y-3">
              ...
              <motion.button
                ...
                className="w-full bg-[#10706B] text-white font-bold py-4 rounded-xl hover:bg-[#0D5C58] transition-all shadow-lg shadow-[#10706B]/20 text-sm tracking-[0.1em] uppercase mt-2 flex items-center justify-center"
              >
