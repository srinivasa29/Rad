...
import { Activity, Bitcoin, DollarSign, Euro, TrendingUp } from 'lucide-react';

const BAR_CONFIG = Array.from({ length: 20 }, (_, index) => ({
  height: 24 + ((index * 11) % 60),
  delay: (index % 5) * 0.2,
  duration: 2 + (index % 4) * 0.35,
}));

const MarketBar = ({ height, delay, duration, index }) => (
  <motion.div
    initial={{ height: '20%' }}
...
const OrbitItem = ({ children, radius, speed, startAngle = 0 }) => (
  <motion.div
    ...
    <div className="relative w-full h-[500px] flex items-end justify-center perspective-[1200px] overflow-visible">
      ...
      >
        ...
        <div className="absolute flex items-end justify-center w-[300px] h-[300px] -translate-x-1/2 -translate-y-1/2" style={{ transform: 'rotateX(-60deg)' }}>
          <div className="flex items-end gap-1 h-32 w-full justify-center opacity-80" style={{ transform: 'translateY(50px)' }}>
            {BAR_CONFIG.map((bar, i) => (
              <MarketBar key={`${bar.height}-${i}`} index={i} height={bar.height} delay={bar.delay} duration={bar.duration} />
            ))}
          </div>
        </div>
        ...
        <div className="absolute pointer-events-none -translate-x-1/2 -translate-y-1/2">
          ...
          >
            <img src="/radar-logo-final.jpg" alt="Radar Market System" className="w-full h-full object-cover rounded-full opacity-90" />

            <div className="absolute bottom-0 w-full h-20 bg-emerald-500/20 blur-xl rounded-full" style={{ transform: 'translateY(50%) rotateX(-90deg)' }} />
          </motion.div>
        </div>
      </div>
    </div>
