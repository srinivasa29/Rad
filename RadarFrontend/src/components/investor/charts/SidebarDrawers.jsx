...
// ── Watchlist Drawer ─────────────────────────────────────────────────────────────
export const WatchlistDrawer = ({ symbol, isDark, mode = 'investor' }) => {
  ...
  const fetchWatchlist = async () => {
    setLoading(true);
    try {
      const res = await api.get('/watchlist', { params: { mode } }).catch(() => null);
      if (res && res.data) {
        const lists = Array.isArray(res.data) ? res.data : [res.data];
        if (lists.length > 0) {
          const list = lists[0];
  ...
  const onAdd = async () => {
    setIsAdding(true);
    try {
      let res = await api.get('/watchlist', { params: { mode } }).catch(() => null);
      let lists = res && res.data ? (Array.isArray(res.data) ? res.data : [res.data]) : [];
      let list = lists.length > 0 ? lists[0] : null;
      
      if (!list || !list._id) {
        // Fallback: If no watchlist exists for this mode, create one first
        const createRes = await api.post('/watchlist', { 
          name: mode === 'investor' ? 'Investor Portfolio' : 'Research Watchlist', 
          mode 
        }).catch(() => null);
  ...
  const onRemove = async (s) => {
    try {
      ...
      let list = lists.length > 0 ? lists[0] : null;
      if (list && list._id) {
        await api.delete(`/watchlist/${list._id}/remove/${encodeURIComponent(s)}`);
  ...
  const cleanSymbol = (sym) => String(sym || '').replace(/\.(NS|BO)$/i, '').toUpperCase();
  const isInWatchlist = watchlist.some(s => cleanSymbol(s) === cleanSymbol(symbol));
            ...
            {watchlist.map((s) => (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <p className={`text-xs font-bold ${isDark ? 'text-slate-200' : 'text-slate-800'}`}>{s.replace(/\.(NS|BO)$/i, '')}</p>
                </div>
                <button 
                  onClick={() => onRemove(s)}
