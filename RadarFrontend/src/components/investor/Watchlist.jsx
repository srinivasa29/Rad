    ...
    useEffect(() => {
        ...
        const load = async (isBackground = false) => {
            ...
            try {
                ...
                let primaryWatchlist = null;
                if (Array.isArray(wlData) && wlData.length > 0) {
                    primaryWatchlist = wlData[0];
                    ...
                    if (created?._id || created?.id) {
                        primaryWatchlist = created;
                        setWatchlistId(created._id || created.id);
                    }
                }

                const rawSymbols = (primaryWatchlist
                    ? (primaryWatchlist.items ?? primaryWatchlist.symbols ?? primaryWatchlist.stocks ?? []).map(extractSymbol)
                    : []).filter(Boolean);
